...
Saved: 2026-04-10
Preservation note: The originally shared link and an Anthropic/Claude path variant were access-restricted in direct fetch. This archive preserves the accessible official Claude blog version available at the source URL above. The shared title phrasing referred to Sonnet and Opus specifically; the accessible official page title is broader and is preserved as the source title.
...
- Date: April 9, 2026
- Reading time: 5 min
- Share link: https://claude.com/blog/the-advisor-strategy
...
In Anthropic’s evaluations, Sonnet with Opus as an advisor showed a 2.7 percentage point increase on SWE-bench Multilingual over Sonnet alone, while reducing cost per agentic task by 11.9%.

## The advisor tool

Anthropic is bringing the advisor strategy to its API with the advisor tool, a server-side tool which Sonnet and Haiku know to invoke when they need guidance or help with a specific task.

In Anthropic’s evaluations, Sonnet with an Opus advisor improved scores across BrowseComp and Terminal-Bench 2.0 benchmarks while costing less per task than Sonnet alone.
...
Declare `advisor_20260301` in your Messages API request, and the model handoff happens inside a single `/v1/messages` request with no extra round-trips or context management. The executor model decides when to invoke it. When it does, Anthropic routes curated context to the advisor model, returns the plan, and the executor continues within the same request.
...
Advisor tokens are reported separately in the usage block.

### Pricing

Advisor tokens are billed at the advisor model's rates, while executor tokens are billed at the executor model's rates. Since the advisor only generates a short plan, typically 400 to 700 text tokens, while the executor handles the full output at its lower rate, the overall cost stays well below running the advisor model end-to-end.
...
- Add `advisor_20260301` to your Messages API request
...
- SWE-bench Multilingual: Sonnet 4.6 solo used adaptive thinking. Sonnet 4.6 + Advisor used Anthropic’s suggested system prompt for coding with thinking turned off. Both runs used high effort with bash and file editing tools. Scores are averaged over five trials of 300 problems across nine languages. Opus 4.6 was used as the advisor model in all runs.
- BrowseComp: All runs used thinking turned off with web search and web fetch tools. Sonnet 4.6 runs used medium effort. Sonnet 4.6 + Advisor used Anthropic’s suggested system prompt for coding; Haiku 4.5 + Advisor did not. No programmatic tool calling or context compaction. Scores are based on 1,266 problems with one attempt per problem. Opus 4.6 was used as the advisor model in all runs.
- Terminal-Bench 2.0: All runs used thinking turned off with bash and file editing tools. Sonnet 4.6 runs used medium effort. Neither advisor run used Anthropic’s suggested system prompt for coding. Each task ran in an isolated pod with 3× resource allocation and a 1× timeout. Scores are averaged over five attempts per task across 89 tasks. Opus 4.6 was used as the advisor model in all runs.
